...
from src.datamodules.multimodal_datamodule import MultimodaDataModule, PTDataModule
...
def training(cfg: DictConfig, idx_cv: int, date=""):
    ...
    if cfg.anchoring:
        ...
        if cfg.checkpoints:
            ...
            callbacks1.append(
                ModelCheckpoint(
                    save_last=True,
                    dirpath=f"{cfg.paths.misc}/checkpoints/adapt_PT/anchoring",
                    filename=name,
                    every_n_epochs=1,
                    verbose=True,
        ...
        # Datamodule structure
        datamodule = PTDataModule(cfg=cfg, cv=idx_cv)
    ...
    if cfg.clf:
        ...
        # Datamodule structure
        datamodule_clf = PTDataModule(cfg=cfg, cv=idx_cv, stage="classification")
...
@hydra.main(version_base="1.2", config_path="configs", config_name="PT_ADAPT.yaml")
