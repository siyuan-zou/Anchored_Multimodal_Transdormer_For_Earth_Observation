...
class VideoDataset(Dataset):
    ...
    def _get_videos_path(cfg, ids, labels):
        ...
        # how can get video path use frames path
        video_paths = os.path.join(cfg.paths.data, cfg.dataset.video_root)
        directories = []
        for root, dirs, files in os.walk(video_paths):
            for file in files:
                if file[-3:] == "mp4":
                    directories.append(os.path.join(root, file))

        for directory in directories:
            # directory should be form of id_task.mp4 but couldn't ! because there are folders in path_frames!
            # id_file = directory.split(".")[0] # id_file = id_task
            id_file = directory.split("/")[-1].split(".")[0]
            ## attention make sure we want to do the split based on files and not ids.
            if id_file in ids and id_file in labels: # the id_file is in the list of ids (to extract) and labels (has data) 
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if file.endswith(".mp4"):
                launches.append(file.split(".")[0])

    # what's inside launches
