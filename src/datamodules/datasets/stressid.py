...
class StressID(torch.utils.data.Dataset):
    ...
    def get_videos(launches: np.array, labels: np.array, path_features: str):
        ...
        launches : np.array (index of the labels_modalities.csv, 6k5f_Stroop)
            List of videos to extract features from.
        labels : read labels_modalities.csv
        """
        values, ids, ys = [], [], []
        ...
        for line in content:
            line = line.split(":")
            values.append(np.asarray(line[5][1:-3].split(", "), dtype=np.float32)) # list of feature values
            ids.append("_".join(line[0:3])) # 6k5f_Stroop_0
            ys.append(float(line[3]))
        data = (np.stack(values, axis=0), np.asarray(ys), np.asarray(ids))
        video_ids = ["_".join(x[0:2]) for x in np.char.split(data[2], sep="_")] # 6k5f_Stroop
        idx_to_keep = np.where(np.isin(video_ids, launches))[0]
        features = data[0][idx_to_keep]
        ids = data[2][idx_to_keep]
        # labelize here
        labels = np.asarray([labels["_".join(video.split("_")[:2])] for video in ids]) # 0/1
        final_ids = []
        for id, label in zip(ids, labels):
            final_ids.append(id.replace("_", ":") + ":" + str(label))
        ids = np.asarray(final_ids) # 6k5f_Stroop:0:0
