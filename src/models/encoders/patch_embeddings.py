...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.res:
            ...
            # print(x.shape) # torch.Size([32, 4, 6, 6, 50, 50])
            x = x.flatten(2, 3)
            x = torch.permute(x,(0,2,1,3,4))
            x = x.flatten(0,1)
            # print(x.shape) # torch.Size([1152, 4, 50, 50])
            indices = []
            sizes = []
            # for i in range (len(self.proj)):
            for i, layer in enumerate(self.proj):
                # Break before extra poolings
                if isinstance(layer, nn.MaxPool2d):
                    # 检查 H, W 是否小于等于 1
                    if x.shape[-2] <= 1 or x.shape[-1] <= 1:
                        # print(f"Stopped at layer {i} because H={x.shape[-2]}, W={x.shape[-1]}")
                        break 
            ...
            # return x, indices, sizes # 不再reconstruct
            x = x.view(B, -1)
            # print(x.shape) # torch.Size([B, 9216])
            return x
        else:
            x = self.proj(x)
            if self.flatten:
